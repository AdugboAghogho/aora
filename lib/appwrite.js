...
//         if (Platform.OS === "web") {
//             // Convert File to Blob for Web Upload
//             const response = await fetch(file.uri);
//             const blob = await response.blob();
//             fileToUpload = new File([blob], file.name || `upload.${type === "video" ? "mp4" : "jpg"}`, { type: file.mimeType });
//         } else {
//             // Use File Directly for Native Upload
//             fileToUpload = file;
//         }
    ...
    try {
        ...
        if (Platform.OS === "web") {
            ...
            fileToUpload = new File(
                [blob],
                file.name || `upload.${type === "video" ? "mp4" : "jpg"}`,
